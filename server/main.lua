...
RegisterCommand('ad', function(source, args, rawCommand)
	...
	TriggerClientEvent('chat:addMessage', -1, { args = { _U('ad_prefix', name), args }, color = { 0, 153, 204 } })
	print(('CHAT: %s: %s'):format(name, args))
end, false)
...
RegisterCommand('atwt', function(source, args, rawCommand)
	...
	args = table.concat(args, ' ')
	local name2 = 'Anonymous'
	if Config.EnableESXIdentity then name = GetCharacterName(source) end

	TriggerClientEvent('chat:addMessage', -1, { args = { _U('twt_prefix', name2), args }, color = { 0, 153, 204 } })
	print(('CHAT: %s: %s'):format(name, args))
end, false)
